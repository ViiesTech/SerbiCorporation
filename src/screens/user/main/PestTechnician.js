...
const PestTechnician = ({ route }) => {
  ...
  const [getAppointmentDetail, { data }] = useLazyGetAppointmentDetailQuery();
    ...
    <Container contentStyle={{ paddingBottom: responsiveHeight(5) }}>
      ...
      >
        ...
        <MapViewDirections
          ...
          onReady={result => setEta(Math.round(result.duration))}
        />
      </MapView>
...
//           break;
//       }
//       console.log('DDDDDDDDDD', data?.data?.location);
//       console.log('destination:-', destination);
//       // Live location tracking: update technician position from server data
//       const techLocationObj = data.data?.location || data.data?.locationName;
